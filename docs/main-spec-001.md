...
## Tabs
4. **Drag-reorder tabs** — *[Library]* `@dnd-kit/react` + `@dnd-kit/dom` (not `@dnd-kit/core`/`sortable`/`modifiers`); order is a `fractional-indexing` string recomputed against the new neighbors on drop, not an `arrayMove` reindex.[^6]
5. **Close-button width-freeze (Chrome behavior)** — *[We build]* freeze tab widths on close, reflow on mouse-leave of the strip.[^6]
6. **Overflow (shrink + horizontal scroll)** — *[We build]* CSS + measurement; deferred until needed.

## Window / OS integration
7. **Global shortcut / quick open** — *[Built-in]* Electron `globalShortcut` (fires even when unfocused).[^7]
...
[^6]: Implementation facts (`src/store/useTabs.js`, `src/components/TabBar.jsx`, `electron/db.js`): every open tab keeps a live `Y.Doc` in memory the whole time it's open, not just the active one. A background tab's content never actually changes (only one CodeMirror view exists, always bound to the active tab), but keeping every tab's doc live means its title is available instantly with no re-fetch when it becomes active, and switching tabs is just rebinding CodeMirror to an already-live `ytext`/`undoManager`/`awareness` triple rather than reconstructing them from persisted updates. Order is a `fractional-indexing` string (`generateKeyBetween`), computed renderer-side on create/reorder — `db.js` only stores it, mirroring simple-gtd-2's `reorderTask`/`reorderSection` pattern (recompute one key between new neighbors, never reindex the whole list). Migration gotcha: `ALTER TABLE notes ADD COLUMN "order" TEXT` leaves every pre-existing row `NULL` (no `DEFAULT`), and `null` compares as neither less-than nor greater-than a fractional-indexing string in JS — `electron/db.js`'s `backfillMissingOrders()` assigns real keys (oldest-first) to any `NULL` row on boot, a no-op once nothing matches. Closing the last open tab auto-creates a fresh blank one (`useTabs.js` `closeTab`) — not in the spec explicitly, but a text editor with zero tabs open is an undefined state worth avoiding. Width-freeze/reflow (`TabBar.jsx`): on close, every remaining tab's current `getBoundingClientRect().width` is captured into a `frozenWidths` map and applied as a fixed `flex: 0 0 <px>` (overriding the normal `flex-1` even-share layout) so the tab strip doesn't redistribute the freed space; the map clears on the strip's `mouseleave`, letting tabs reflow back to even widths. Known limitation: Vim mode / read-only lock (footnote 5) is App-level state, not per-tab, so it carries over across tab switches instead of being scoped to the note it was toggled on.

[^7]: Implementation facts (`electron/main.js`): `app.requestSingleInstanceLock()` gates the whole rest of the file — a losing instance must call `app.quit()` and do nothing else, since `app.quit()` doesn't halt script execution by itself; everything else (window creation, IPC handlers, shortcut registration) lives in the `else` branch to avoid a losing instance briefly opening its own window/DB connection first. The winning instance's `second-instance` event and a registered `globalShortcut` both call the same shared toggle function, so a real duplicate-launch attempt and a shortcut press are handled identically — this is what makes an OS-reserved key combo usable at all: Windows claims Win+Space for input-language switching, so Electron can never register it directly (confirmed via testing — `globalShortcut.register` returns `false` for it), but an external launcher relaunching the app on that keypress works instead, since the relaunch attempt's `second-instance` event is the delivery mechanism, not the key registration itself. Exact accelerator choice, the external-launcher script, and final toggle semantics (vs. this doc's "restore last note exactly as it was") are still being finalized — deliberately not detailed here yet, since they're expected to keep changing.
